...
{
	//create shaders
	SHADER_HELPERS::CreateVertexShader(L"Data/shaders/vertexForwardWater.shader", _waterVertexShader, waterVertexShaderByteCode);
	SHADER_HELPERS::CreatePixelShader(L"Data/shaders/pixelForwardWater.shader",   _waterPixelShader, waterPixelShaderByteCode);
...
void WaterShader::Render(std::vector<Mesh*>& waterMeshes)
{
	...
	for (int i = 0; i < numMeshes; i++)
	{
		...
		_simpleClipShaderRefraction->RenderScene(renderer.GetMeshes(SHADER_TYPE::S_REFRACT_OPAQUE),  renderer.GetInstancedModels(INSTANCED_SHADER_TYPE::S_INSTANCED_REFRACT), clipPlaneRefract, true, false);
		...
		_simpleClipShaderReflection->RenderScene(renderer.GetMeshes(SHADER_TYPE::S_CAST_REFLECTION), renderer.GetInstancedModels(INSTANCED_SHADER_TYPE::S_INSTANCED_CAST_REFLECTION), clipPlaneReflect, true);
		...
		// set SRV's
		devCon->PSSetShaderResources(0, 10, t);
		...
		devCon->PSSetShaderResources(0, 10, nullSRV);
	}
}
