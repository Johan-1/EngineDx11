...
{
	//create shaders
	SHADER_HELPERS::CreateVertexShader(L"Data/shaders/vertexForwardPlanar.shader", _planarVertexShader, planarVertexShaderByteCode);
	SHADER_HELPERS::CreatePixelShader(L"Data/shaders/pixelForwardPlanar.shader",   _planarPixelShader, planarPixelShaderByteCode);
...
void PlanarReflectionShader::Render(std::vector<Mesh*>& reflectionMeshes)
{
	...
	for (int i = 0; i < numMeshes; i++)
	{
		...
		_simpleClipShaderReflection->RenderScene(renderer.GetMeshes(SHADER_TYPE::S_CAST_REFLECTION), renderer.GetInstancedModels(INSTANCED_SHADER_TYPE::S_INSTANCED_CAST_REFLECTION), clipPlane, true);
